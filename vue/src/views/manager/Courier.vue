  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table stripe :data="data.tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="commissionName" label="代取员等级" />
        <el-table-column prop="cash" label="代取员钱包" />
        <el-table-column prop="status" label="状态">
          <template v-slot="scope">
            <el-tag type="success" v-if="scope.row.status === '正常'">正常</el-tag>
            <el-tag type="danger" v-if="scope.row.status === '封禁'">封禁</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="200" fixed="right">
          <template v-slot="scope">
            <el-button type="danger" @click="ban(scope.row)">封禁</el-button>
    ...
    <el-dialog title="代取员信息" v-model="data.formVisible" width="40%" destroy-on-close>
...
const ban = (row) => {
  ElMessageBox.confirm('您确认封禁代取员【' + row.name + '】吗？', '封禁确认', { type: 'warning' }).then(res => {
    data.form = JSON.parse(JSON.stringify(row))
    data.form.status = '封禁'
